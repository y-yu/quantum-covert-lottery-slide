...
\begin{frame}
  ...
  \begin{itemize}
    ...
    % \item たとえば$\ket{0} = \Zero, \ket{1} = \One$のように行列で表せる
    % \[
...
\begin{frame}
  ...
  \begin{columns}
    ...
    \begin{column}{0.6\textwidth}
      \begin{itemize}
        \item ``シュレディンガーの猫''で有名なように、量子ビットは$\ket{0}$と$\ket{1}$の
        ``混ざった''ような状態を表現できる
        ...
        \item これらは$\ket{0}, \ket{1}$が観測される確率が
...
\begin{frame}
  ...
  \begin{itemize}
    \item 2つの量子ビットが$\ket{+}$のとき$CZ$ゲートの後で下記のような操作をする
...
\begin{frame}
  ...
  \begin{itemize}
    \item 下記のように$Z$ゲートと$H$ゲートが2量子ビット目へテレポートする
    \begin{itemize}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    シミュレーション結果の図\ref{fig:h_gate_teleportation_histogram}では1量子ビット目は$\ket{0},\ket{1}$がランダムとなっている
  \end{itemize}
    ...
    \begin{column}{0.5\textwidth}
      \begin{figure}
        ...
        \Qcircuit @C=1.0em @R=0.2em @!R {
          ...
          \lstick{\ket{0}_1} & \gate{H} & \ctrl{1}        & \gate{S}\cwx & \qw      & \qw\\
          \lstick{\ket{0}_2} & \gate{H} & \gate{Z}        & \ctrl{1}     & \qw      & \qw\\
          \lstick{\ket{0}_3} & \gate{H} & \gate{Z}\cwx[1] & \gate{Z}     & \gate{H} & \qw \\
...
\begin{frame}
  ...
  \begin{columns}
    ...
    \end{column}
    \begin{column}{0.5\textwidth}
      \alicecallout{+}{まずは$\ket{0}_1$だけ整理してみる}
    \end{column}
  \end{columns}
...
\begin{frame}
  ...
  \alicecallout{-}{$S$ゲートが消えているのがポイント！}  
\end{frame}
...
\begin{frame}
  ...
  \bobcallout{+}{$S$ゲートが2量子ビット目へテレポートされる！}
\end{frame}
  ...
  \begin{enumerate}
    ...
    \begin{figure}
      \scalebox{1.0}{
        \Qcircuit @C=1.0em @R=0.2em @!R {
          \lstick{b}         & \cw      & \cw       & \cw       & \control\cw \\
          \lstick{\ket{0}_1} & \gate{H} & \ctrl{1}  & \gate{S^?} & \gate{S}\cwx & \gate{H} & \meter & \rstick{\rightarrow c_1} \\
                             &          & 
      }}%
    \end{figure}
...
\begin{frame}
  ...
  \alicecallout{-}{2, 3量子ビット目は$a, b$で次のように場合わけされる}  
  ...
  \begin{description}
    ...
    \begin{itemize}
      \item $H$ゲートの相殺により2量子ビット目は$\ket{0}$に確定し$c_2 = 0$\ce{:person_gesturing_ok:}
    \end{itemize}
  \end{description}
\end{frame}
...
\begin{frame}
  ...
  \begin{description}
    ...
    \end{figure}
    \begin{itemize}
      \item $H,Z,H$ゲート操作で2量子ビット目は$\ket{1}$に確定し$c_2 = 1$\ce{:person_gesturing_ok:}
    \end{itemize}
...
\begin{frame}
  ...
  \begin{enumerate}
    ...
    \begin{itemize}
      \item アリスとボブの希望が衝突した場合、ボブは自分に都合がいい$c_2$が観測されたと偽ることができる\ce{:smiling_imp:}
