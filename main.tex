  ...
  \begin{columns}
    \begin{column}{0.5\textwidth}
      \begin{figure}
        ...
        \scalebox{1.0}{
        \Qcircuit @C=1.0em @R=0.2em @!R {
          \lstick{a}         & \cw      & \control\cw  & \\
          \lstick{\ket{0}_1} & \gate{H} & \gate{S}\cwx & \ctrl{1} & \qw      & \qw \\
          \lstick{\ket{0}_2} & \gate{H} & \qw          & \gate{Z} & \ctrl{1} & \qw \\
          \lstick{\ket{0}_3} & \gate{H} & \gate{Z}\cwx[1] & \qw      & \gate{Z} & \qw \\
...
\begin{frame}
  ...
  \begin{figure}
    ...
    \Qcircuit @C=1.0em @R=0.2em @!R {
      ...
      \lstick{\ket{0}_1} & \gate{H} & \gate{S?} & \ctrl{1} & \gate{S}\cwx & \gate{H} & \meter \\%& \control \cw \\
      \lstick{\ket{0}_2} & \gate{H} & \qw       & \gate{Z} & \ctrl{1}     & \qw      & \qw    \\%& \gate{X}\cwx & \meter \\
      \lstick{\ket{0}_3} & \gate{H} & \gate{Z?} & \qw      & \gate{Z}     & \qw      & \qw    %& \gate{X}\cwx
    }}
    \caption{ボブが行う量子操作}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \begin{figure}
      ...
      \Qcircuit @C=1.0em @R=0.2em @!R {
        \lstick{a}         & \cw      & \control\cw \\
        \lstick{\ket{0}_1} & \gate{H} & \gate{S}\cwx & \ctrl{2} & \gate{S}\cwx[1] & \gate{H} & \meter \\
        \lstick{b}         & \cw      & \cw          & \cw      & \control\cw \\
                           &          &              & \dstick{CZ}
      }}
      \vspace{2ex}
    ...
    \begin{columns}
      ...
      \begin{column}{0.5\textwidth}
        \begin{figure}
          ...
          \scalebox{1.0}{
          \Qcircuit @C=1.0em @R=0.2em @!R {
            \lstick{\ket{0}_1} & \gate{H} & \gate{S} & \ctrl{1} & \gate{S} & \gate{H} & \meter \\
                               &          &          & \dstick{CZ}
          }}
          \vspace{2ex}
        ...
        \begin{figure}
          \includegraphics[width=0.5\textwidth]{./img/a_1_histogram.pdf}
        \end{figure}
      \end{column}
    \end{columns}
  
    \item このように常に$\ket{0}, \ket{1}$が生じるようになる
  \end{itemize}
\end{frame}
    ...
    \begin{columns}
      \begin{column}{0.6\textwidth}
        ...
        \begin{figure}
          ...
          \scalebox{1.0}{
          \Qcircuit @C=1.0em @R=0.2em @!R {
            \lstick{\ket{0}_1} & \gate{H} & \gate{S} & \ctrl{1}    & \gate{H} & \meter \\
                               &          &          & \dstick{CZ}
          }}
          \vspace{2ex}
