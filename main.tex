...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item<+-> アリス・ボブの希望が割れた場合は結果が50\%のランダムとなりOK\ce{:person_gesturing_ok:}
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \pause
  \begin{itemize}
    \item<+-> 二人の希望が衝突した場合、ボブは自分に都合がいい$c_2$が観測されたと偽ることができる\ce{:smiling_imp:}
  ...
  \begin{figure}
    \scalebox{0.8}{
      \Qcircuit @C=1.0em @R=0.2em @!R {
        ...
        \lstick{\ket{0}_4} & \gate{H} & \gate{Z^{x_3}} & \qw              & \gate{Z}         & \gate{H} & \qw & \gate{X}\cwx[-1] & \meter & \rstick{\rightarrow c_5}\\
      }}
