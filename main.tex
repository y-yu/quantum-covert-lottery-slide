    ...
    \begin{column}{0.8\textwidth}
      \begin{itemize}
        ...
        \begin{enumerate}
          ...
          \begin{description}
            \item[$\{\ket{\heartcard}, \ket{\clubcard}\}$で測定した時]
...
1. アリスがSゲートをいれて（x = 1）、Zゲートはいれてない（a = 0 …… 後手）
    i. ボブはSゲートをいれずに1 qubit目を{|+>, |->}で測定（b = 1 …… 先手） …… ランダム（測定結果 => x'）
      a. 1 qubitの測定が0
        1. ボブはこれをアリスへ通知
        2. アリスがx = 1をボブへ通知
        3. ボブは2 qubit目を{SH|0>, SH|1>}で測定
      b. 1 qubitの測定が1
        1. ボブはこれをアリスへ通知
        2. アリスがx = 1をボブへ通知
        3. ボブは2 qubit目を{SHX|0>, SHX|1>}で測定

2. アリスがSゲートをいれず（x = 0）、Zゲートはいれてない（a = 0 …… 後手）
  i. ボブはSゲートをいれて1 qubit目を{S|+>, S|->}で測定（b = 1 …… 先手） …… ランダム（測定結果 => x'）
    a. 1 qubitの測定が0
      1. ボブはこれをアリスへ通知
      2. アリスがx = 0をボブへ通知
      3. ボブは2 qubit目を{SH|0>, SH|1>}で測定
...
3. アリスがSゲートをいれて（x = 1）、Zゲートはいれてない（a = 1 …… 先手）
  i. ボブはSゲートをいれずに1 qubit目を{|+>, |->}で測定（b = 1 …… 先手） …… ランダム（測定結果 => x'）
    a. 1 qubitの測定が0
      1. ボブはこれをアリスへ通知
      2. アリスがx = 1をボブへ通知
